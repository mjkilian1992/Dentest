...
describe('MainQuestionViewerCtrl', function(){
    ...
    var allQuestionsResponsePage2 = {
        count: 4,
        next: "https://dentest.com/questions/?format=json&page=3&page_size=1",
        previous: "https://dentest.com/questions/?format=json&page=1&page_size=1",
        results: [
            {
                id: 1,
                subtopic: {
    ...
    //inject dependencies
    beforeEach(inject(function ($location, $rootScope, $q, $httpBackend, REST_BASE_URL) {
    ...
    describe('Instantiation', inject(function($controller){
        ...
        it('should display all questions if not topic/subtopic was provided', function () {
            ...
            expect(ctrl.questions).toEqual(allQuestionsResponsePage1.results);
            expect(qService.getQuestions()).toHaveBeenCalledWith(1);

        });

        it('should pull down all questions in a topic if only the topic was provided', function () {
        ...
        it('should pull down all questions in a subtopic if the topic/subtopic was provided', function () {
    ...
    describe('Pagination', inject(function ($controller) {
        it('should store the total number of pages for the current results', function () {
            ctrl = $controller('MainQuestionViewerCtrl');
            expect(ctrl.no_of_pages()).toEqual();
        });


        it('should pull down the next page if next_page is called', function () {
            ...
            backend.expectGET(baseURL + '/questions/' + buildParams(2, 1)).respond(200, allQuestionsResponsePage1);
            ctrl.next_page();
            backend.flush();
            expect(ctrl.questions).toEqual(allQuestionsResponsePage1.results);
        ...
        it('should pull down the previous page if previous_page is called', function () {
            //Mock a first request to set up pagination info
            backend.expectGET(baseURL + '/questions/' + buildParams(1, 1)).respond(200, allQuestionsResponsePage1);
            ctrl = $controller('MainQuestionViewerCtrl');
            backend.flush();
            ...
            backend.expectGET(baseURL + '/questions/' + buildParams(2, 1)).respond(200, allQuestionsResponsePage2);
            ctrl.next_page();
